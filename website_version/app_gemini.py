...
def commander_start():
    ...
    if current_round_index == -1 or current_round_index >= len(all_rounds_pairings):
        ...
        emit('message', {'msg': 'A tournament is already in progress or has unfinished rounds.'}, room='commander')
...
def start_game_tournament():
    ...
    if not waiting_players:
        socketio.emit('message', {'msg': 'No players to start a tournament!'}, room='commander')
        return
    ...
    socketio.emit('message', {'msg': f'Tournament started with {len(all_rounds_pairings)} rounds.'}, room='commander')
    play_next_round()
...
def play_next_round():
    ...
    if current_round_index >= len(all_rounds_pairings):
        socketio.emit('message', {'msg': 'Tournament finished!'}, room='commander')
        socketio.emit('message', {'msg': 'All rounds complete! Thanks for playing.'}, broadcast=True)
    ...
    socketio.emit('message', {'msg': f'Starting Round {current_round_index + 1} with {len(current_round_pairings)} games.'}, room='commander')
    ...
    for p1, p2 in current_round_pairings:
        if p1 not in players or p2 not in players:
            ...
            if p1 in players:
                socketio.emit('message', {'msg': 'Your opponent disconnected. Waiting for next round or tournament restart.'}, room=p1)
                players[p1]['ready_for_next_game'] = True # Mark as ready to be re-matched
            if p2 in players:
                socketio.emit('message', {'msg': 'Your opponent disconnected. Waiting for next round or tournament restart.'}, room=p2)
        ...
        socketio.emit('start', {'opponent': p2[:4], 'game_log': game_log, 'round': current_round_index + 1}, room=p1)
        socketio.emit('start', {'opponent': p1[:4], 'game_log': game_log, 'round': current_round_index + 1}, room=p2)
        socketio.emit('message', {'msg': 'Your turn! Choose a move:'}, room=p1)
        socketio.emit('message', {'msg': 'Waiting for opponent...'}, room=p2)

    if not current_round_pairings: # If all pairs were None (e.g., only one player left)
        socketio.emit('message', {'msg': f'Round {current_round_index + 1} has no active games. Advancing to next round.'}, room='commander')
...
def handle_move(data):
    ...
    if player_data.get('ready_for_next_game'):
        emit('message', {'msg': 'Your game has concluded. Waiting for next round.'}, room=sid)
        return

    opponent_sid = player_data.get('opponent')
    if not opponent_sid or opponent_sid not in players:
        emit('message', {'msg': 'No opponent found or opponent disconnected.'}, room=sid)
    ...
    if move_symbol == 'x':
        ...
        emit('message', {'msg': 'Game Over! You took the last item. Waiting for next round...'}, room=sid)
        emit('message', {'msg': 'Game Over! Your opponent took the last item. Waiting for next round...'}, room=opponent_sid)
...
def check_round_completion():
    ...
    if not games_in_current_round: # If there were no active games in this round (e.g., all players disconnected)
        all_games_completed_in_round = True
    else:
    ...
    if all_games_completed_in_round:
        socketio.emit('message', {'msg': f'Round {current_round_index + 1} completed!'}, room='commander')
        current_round_index += 1
